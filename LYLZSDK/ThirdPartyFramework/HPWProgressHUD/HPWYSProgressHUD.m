...
#error HPWYSProgressHUD is ARC only. Either turn on ARC for the project or use -fobjc-arc flag
#endif

#import "HPWYSProgressHUD.h"
#import "HPWYSIndefiniteAnimatedView.h"
#import "HPWYSProgressAnimatedView.h"
#import "HPWYSRadialGradientLayer.h"

NSString * const HPWYSProgressHUDDidReceiveTouchEventNotification = @"HPWYSProgressHUDDidReceiveTouchEventNotification";
NSString * const HPWYSProgressHUDDidTouchDownInsideNotification = @"HPWYSProgressHUDDidTouchDownInsideNotification";
NSString * const HPWYSProgressHUDWillDisappearNotification = @"HPWYSProgressHUDWillDisappearNotification";
NSString * const HPWYSProgressHUDDidDisappearNotification = @"HPWYSProgressHUDDidDisappearNotification";
NSString * const HPWYSProgressHUDWillAppearNotification = @"HPWYSProgressHUDWillAppearNotification";
NSString * const HPWYSProgressHUDDidAppearNotification = @"HPWYSProgressHUDDidAppearNotification";

NSString * const HPWYSProgressHUDStatusUserInfoKey = @"HPWYSProgressHUDStatusUserInfoKey";

static const CGFloat HPWYSProgressHUDParallaxDepthPoints = 10.0f;
static const CGFloat HPWYSProgressHUDUndefinedProgress = -1;
static const CGFloat HPWYSProgressHUDDefaultAnimationDuration = 0.15f;
static const CGFloat HPWYSProgressHUDVerticalSpacing = 12.0f;
static const CGFloat HPWYSProgressHUDHorizontalSpacing = 12.0f;
static const CGFloat HPWYSProgressHUDLabelSpacing = 8.0f;


@interface HPWYSProgressHUD ()

@property (nonatomic, strong) NSTimer *fadeOutTimer;

@property (nonatomic, strong) UIControl *controlView;
@property (nonatomic, strong) UIView *backgroundView;
@property (nonatomic, strong) HPWYSRadialGradientLayer *backgroundRadialGradientLayer;
...
@property (nonatomic, strong) HPWYSProgressAnimatedView *ringView;
@property (nonatomic, strong) HPWYSProgressAnimatedView *backgroundRingView;
...
@implementation HPWYSProgressHUD {
    BOOL _isInitializing;
}

+ (HPWYSProgressHUD*)sharedView {
    static dispatch_once_t once;
    
    static HPWYSProgressHUD *sharedView;
#if !defined(HPWYS_APP_EXTENSIONS)
...
+ (void)setDefaultStyle:(HPWYSProgressHUDStyle)style {
    [self sharedView].defaultStyle = style;
    [self sharedView].hudView.alpha = style != HPWYSProgressHUDStyleCustom ? 1.0f : 0.0f;
}

+ (void)setDefaultMaskType:(HPWYSProgressHUDMaskType)maskType {
    [self sharedView].defaultMaskType = maskType;
}

+ (void)setDefaultAnimationType:(HPWYSProgressHUDAnimationType)type {
...
+ (void)setForegroundColor:(UIColor*)color {
    ...
    [self setDefaultStyle:HPWYSProgressHUDStyleCustom];
}
...
+ (void)showWithMaskType:(HPWYSProgressHUDMaskType)maskType {
    HPWYSProgressHUDMaskType existingMaskType = [self sharedView].defaultMaskType;
...
+ (void)showWithStatus:(NSString*)status {
    [self showProgress:HPWYSProgressHUDUndefinedProgress status:status];
}

+ (void)showWithStatus:(NSString*)status maskType:(HPWYSProgressHUDMaskType)maskType {
    HPWYSProgressHUDMaskType existingMaskType = [self sharedView].defaultMaskType;
    [self setDefaultMaskType:maskType];
...
+ (void)showProgress:(float)progress maskType:(HPWYSProgressHUDMaskType)maskType {
    HPWYSProgressHUDMaskType existingMaskType = [self sharedView].defaultMaskType;
    [self setDefaultMaskType:maskType];
...
+ (void)showProgress:(float)progress status:(NSString*)status maskType:(HPWYSProgressHUDMaskType)maskType {
    HPWYSProgressHUDMaskType existingMaskType = [self sharedView].defaultMaskType;
    [self setDefaultMaskType:maskType];
...
+ (void)showInfoWithStatus:(NSString*)status maskType:(HPWYSProgressHUDMaskType)maskType {
    HPWYSProgressHUDMaskType existingMaskType = [self sharedView].defaultMaskType;
    [self setDefaultMaskType:maskType];
...
+ (void)showSuccessWithStatus:(NSString*)status maskType:(HPWYSProgressHUDMaskType)maskType {
    HPWYSProgressHUDMaskType existingMaskType = [self sharedView].defaultMaskType;
    [self setDefaultMaskType:maskType];
...
+ (void)showErrorWithStatus:(NSString*)status maskType:(HPWYSProgressHUDMaskType)maskType {
    HPWYSProgressHUDMaskType existingMaskType = [self sharedView].defaultMaskType;
    [self setDefaultMaskType:maskType];
...
+ (void)showImage:(UIImage*)image status:(NSString*)status maskType:(HPWYSProgressHUDMaskType)maskType {
    HPWYSProgressHUDMaskType existingMaskType = [self sharedView].defaultMaskType;
    [self setDefaultMaskType:maskType];
...
- (instancetype)initWithFrame:(CGRect)frame {
    if((self = [super initWithFrame:frame])) {
        ...
        // Set default values
        _defaultMaskType = HPWYSProgressHUDMaskTypeNone;
        _defaultStyle = HPWYSProgressHUDStyleLight;
        _defaultAnimationType = HPWYSProgressHUDAnimationTypeFlat;
        _minimumSize = CGSizeZero;
        _font = [UIFont preferredFontForTextStyle:UIFontTextStyleSubheadline];
        
        NSBundle *bundle = [NSBundle bundleForClass:[HPWYSProgressHUD class]];
        NSURL *url = [bundle URLForResource:@"LYTChatProgressHUD" withExtension:@"bundle"];
        ...
        _fadeInAnimationDuration = HPWYSProgressHUDDefaultAnimationDuration;
        _fadeOutAnimationDuration = HPWYSProgressHUDDefaultAnimationDuration;
        
        _maxSupportedWindowLevel = UIWindowLevelNormal;
        
        // Accessibility support
        self.accessibilityIdentifier = @"LYTChatProgressHUD";
        self.accessibilityLabel = @"LYTChatProgressHUD";
...
- (void)updateHUDFrame {
    ...
    hudWidth = HPWYSProgressHUDHorizontalSpacing + MAX(labelWidth, contentWidth) + HPWYSProgressHUDHorizontalSpacing;
    
    // |-spacing-content-(labelSpacing-label-)spacing-|
    hudHeight = HPWYSProgressHUDVerticalSpacing + labelHeight + contentHeight + HPWYSProgressHUDVerticalSpacing;
    if(self.statusLabel.text && (imageUsed || progressUsed)){
        // Add spacing if both content and label are used
        hudHeight += HPWYSProgressHUDLabelSpacing;
    }
    ...
    CGFloat centerY;
    if(self.statusLabel.text) {
        CGFloat yOffset = MAX(HPWYSProgressHUDVerticalSpacing, (self.minimumSize.height - contentHeight - HPWYSProgressHUDLabelSpacing - labelHeight) / 2.0f);
    ...
    if(self.progress != HPWYSProgressHUDUndefinedProgress) {
    ...
    // Label
    if(imageUsed || progressUsed) {
        centerY = CGRectGetMaxY(imageUsed ? self.imageView.frame : self.indefiniteAnimatedView.frame) + HPWYSProgressHUDLabelSpacing + labelHeight / 2.0f;
    } else {
        centerY = CGRectGetMidY(self.hudView.bounds);
    }
...
- (void)updateMotionEffectForXMotionEffectType:(UIInterpolatingMotionEffectType)xMotionEffectType yMotionEffectType:(UIInterpolatingMotionEffectType)yMotionEffectType {
    ...
    effectX.minimumRelativeValue = @(-HPWYSProgressHUDParallaxDepthPoints);
    effectX.maximumRelativeValue = @(HPWYSProgressHUDParallaxDepthPoints);
    
    UIInterpolatingMotionEffect *effectY = [[UIInterpolatingMotionEffect alloc] initWithKeyPath:@"center.y" type:yMotionEffectType];
    effectY.minimumRelativeValue = @(-HPWYSProgressHUDParallaxDepthPoints);
    effectY.maximumRelativeValue = @(HPWYSProgressHUDParallaxDepthPoints);
...
#else
            // If HPWYSProgressHUD ist used inside an app extension add it to the given view
...
- (NSDictionary*)notificationUserInfo {
    return (self.statusLabel.text ? @{HPWYSProgressHUDStatusUserInfoKey : self.statusLabel.text} : nil);
}
...
#if !defined(HPWYS_APP_EXTENSIONS) && TARGET_OS_IOS
    self.frame = [[[UIApplication sharedApplication] delegate] window].bounds;
    UIInterfaceOrientation orientation = UIApplication.sharedApplication.statusBarOrientation;
#elif !defined(HPWYS_APP_EXTENSIONS) && !TARGET_OS_IOS
...
- (void)controlViewDidReceiveTouchEvent:(id)sender forEvent:(UIEvent*)event {
    [[NSNotificationCenter defaultCenter] postNotificationName:HPWYSProgressHUDDidReceiveTouchEventNotification
                                                        object:self
    ...
    if(CGRectContainsPoint(self.hudView.frame, touchLocation)) {
        [[NSNotificationCenter defaultCenter] postNotificationName:HPWYSProgressHUDDidTouchDownInsideNotification
                                                            object:self
                                                          userInfo:[self notificationUserInfo]];
    }
}
...
- (void)showProgress:(float)progress status:(NSString*)status {
    __weak HPWYSProgressHUD *weakSelf = self;
    [[NSOperationQueue mainQueue] addOperationWithBlock:^{
        __strong HPWYSProgressHUD *strongSelf = weakSelf;
        if(strongSelf){
...
- (void)showImage:(UIImage*)image status:(NSString*)status duration:(NSTimeInterval)duration {
    ...
    [[NSOperationQueue mainQueue] addOperationWithBlock:^{
        ...
        if(strongSelf){
            ...
            strongSelf.progress = HPWYSProgressHUDUndefinedProgress;
            [strongSelf cancelRingLayerAnimation];
            [strongSelf cancelIndefiniteAnimatedViewAnimation];
...
- (void)showStatus:(NSString*)status {
    ...
    if(self.defaultMaskType != HPWYSProgressHUDMaskTypeNone) {
    ...
    if(self.hudView.alpha != 1.0f){
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:HPWYSProgressHUDWillAppearNotification
                                                            object:self
                                                          userInfo:[self notificationUserInfo]];
...
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 80000
            if(self.defaultStyle != HPWYSProgressHUDStyleCustom){
                // Fade in effect
                UIBlurEffectStyle blurEffectStyle = self.defaultStyle == HPWYSProgressHUDStyleDark ? UIBlurEffectStyleDark : UIBlurEffectStyleExtraLight;
        ...
        __block void (^completionBlock)(void) = ^{
            ...
            if(self.hudView.alpha == 1.0f){
                ...
                // Post notification to inform user
                [[NSNotificationCenter defaultCenter] postNotificationName:HPWYSProgressHUDDidAppearNotification
                                                                    object:self
                                                                  userInfo:[self notificationUserInfo]];
            }
...
- (void)dismissWithDelay:(NSTimeInterval)delay completion:(HPWYSProgressHUDDismissCompletion)completion {
    ...
    [[NSOperationQueue mainQueue] addOperationWithBlock:^{
        ...
        if(strongSelf){
            // Post notification to inform user
            [[NSNotificationCenter defaultCenter] postNotificationName:HPWYSProgressHUDWillDisappearNotification
                                                                object:nil
            ...
            __block void (^completionBlock)(void) = ^{
                ...
                if(strongSelf.hudView.alpha == 0.0f){
                    ...
                    // Post notification to inform user
                    [[NSNotificationCenter defaultCenter] postNotificationName:HPWYSProgressHUDDidDisappearNotification
                                                                        object:strongSelf
                                                                      userInfo:[strongSelf notificationUserInfo]];
...
- (UIView*)indefiniteAnimatedView {
    ...
    if(self.defaultAnimationType == HPWYSProgressHUDAnimationTypeFlat){
        // Check if spinner exists and is an object of different class
        if(_indefiniteAnimatedView && ![_indefiniteAnimatedView isKindOfClass:[HPWYSIndefiniteAnimatedView class]]){
            [_indefiniteAnimatedView removeFromSuperview];
            _indefiniteAnimatedView = nil;
        }
        
        if(!_indefiniteAnimatedView){
            _indefiniteAnimatedView = [[HPWYSIndefiniteAnimatedView alloc] initWithFrame:CGRectZero];
        }
        
        // Update styling
        HPWYSIndefiniteAnimatedView *indefiniteAnimatedView = (HPWYSIndefiniteAnimatedView*)_indefiniteAnimatedView;
...
- (HPWYSProgressAnimatedView*)ringView {
    if(!_ringView) {
        _ringView = [[HPWYSProgressAnimatedView alloc] initWithFrame:CGRectZero];
    }
...
- (HPWYSProgressAnimatedView*)backgroundRingView {
    if(!_backgroundRingView) {
        _backgroundRingView = [[HPWYSProgressAnimatedView alloc] initWithFrame:CGRectZero];
...
- (UIColor*)foregroundColorForStyle {
    if(self.defaultStyle == HPWYSProgressHUDStyleLight) {
        return [UIColor blackColor];
    } else if(self.defaultStyle == HPWYSProgressHUDStyleDark) {
...
- (UIColor*)backgroundColorForStyle {
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 80000
    // On iOS 8 and HPWYSProgressHUDStyleLight / HPWYSProgressHUDStyleDark the
    // the background color is set via a UIVisualEffectsView
    return self.defaultStyle == HPWYSProgressHUDStyleCustom ? self.backgroundColor : [UIColor clearColor];
#else
    if(self.defaultStyle == HPWYSProgressHUDStyleLight) {
        return [UIColor whiteColor];
    } else if(self.defaultStyle == HPWYSProgressHUDStyleDark) {
...
-(UIView *)backgroundView {
    ...
    switch (self.defaultMaskType) {
        case HPWYSProgressHUDMaskTypeCustom:
        case HPWYSProgressHUDMaskTypeBlack:{
            ...
            _backgroundView.backgroundColor = self.defaultMaskType == HPWYSProgressHUDMaskTypeCustom ? self.backgroundLayerColor : [UIColor colorWithWhite:0 alpha:0.4];
            break;
        }
        case HPWYSProgressHUDMaskTypeGradient:{
            if(!_backgroundRadialGradientLayer){
                _backgroundRadialGradientLayer = [HPWYSRadialGradientLayer layer];
            }
    ...
    if(_backgroundRadialGradientLayer){
        ...
        _backgroundRadialGradientLayer.HPWYSgradientCenter = gradientCenter;
    }
...
- (void)setDefaultAnimationType:(HPWYSProgressHUDAnimationType)animationType {
