...
@implementation LYUnOpenFileView
- (instancetype)init
{
    if (self = [super init]) {
        [self setupSubView];
...
- (void)setupSubView
{
    ...
    [_openOtherBtn setBackgroundImage:[UIImage imageWithColor:color01a] forState:UIControlStateNormal];
...
//#warning SD WEB IMAGE SETTER
        [_fileImage hpwys_setImageWithURL:[NSURL URLWithString:_model.fileUrl] placeholderImage:_model.image];
...
- (void)layoutSubviews
{
    ...
    [_descriptation hpwys_makeConstraints:^(HPWYSConstraintMaker *make) {
        make.top.equalTo(self.fileSize.hpwys_bottom).offset(40);
        make.centerX.equalTo(self);
    }];
    [_openOtherBtn hpwys_makeConstraints:^(HPWYSConstraintMaker *make) {
        make.bottom.equalTo(self).offset(-20);
        make.height.hpwys_equalTo(44);
