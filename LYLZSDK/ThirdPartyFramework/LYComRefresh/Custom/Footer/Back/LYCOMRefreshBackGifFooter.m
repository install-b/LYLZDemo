#import "LYCOMRefreshBackGifFooter.h"

@interface LYCOMRefreshBackGifFooter()
...
@implementation LYCOMRefreshBackGifFooter
#pragma mark - 懒加载
- (UIImageView *)gifView
{
    if (!_gifView) {
...
- (void)setPullingPercent:(CGFloat)pullingPercent
{
    ...
    NSArray *images = self.stateImages[@(LYCOMRefreshStateIdle)];
    if (self.state != LYCOMRefreshStateIdle || images.count == 0) return;
...
- (void)setState:(LYCOMRefreshState)state
{
    ...
    // 根据状态做事情
    if (state == LYCOMRefreshStatePulling || state == LYCOMRefreshStateRefreshing) {
    ...
    } else if (state == LYCOMRefreshStateIdle) {
        self.gifView.hidden = NO;
